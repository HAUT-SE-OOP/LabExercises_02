...
const String &String::operator+( const String &left)/* Write the header for the operator+ member function */
{
   String temp;/* Declare a temporary String variable named temp */
   temp.length = left.length+length;/* Set temp抯 length to be the sum of the two argument Strings?lengthes */
   char*temptr = new char[ temp.length + 1 ];  /* Allocate memory for temp.length + 1 chars and assign the pointer to temp.sPtr */
   assert( temp.sPtr != 0 ); // terminate if memory not allocated
   strcpy( temptr,sPtr);
   /* Copy the left String argument抯 contents into temp.sPtr */
   strcat(temptr+length,left.sPtr);
    delete []sPtr;
   sPtr=temptr;
   length=temp.length;
   /* Write a call to strcat to concatenate the string in right
      onto the end of the string in temp */
   return *this;/* Return the temporary String */
} // end function operator+

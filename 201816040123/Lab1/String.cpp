...
//delete [] temp.sPtr;
   temp.length=length+right.length;/* Set temp抯 length to be the sum of the two argument Strings?lengthes */
   temp.sPtr=new char [ temp.length+1 ];/* Allocate memory for temp.length + 1 chars and assign the pointer to temp.sPtr */
   assert( sPtr != 0 ); // terminate if memory not allocated
   strcpy(temp.sPtr,sPtr);/* Copy the left String argument抯 contents into temp.sPtr */
   strcat(temp.sPtr,right.sPtr);/* Write a call to strcat to concatenate the string in right

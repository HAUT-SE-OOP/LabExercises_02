...
bool HugeInt::operator==( HugeInt &op2 )
{
    ...
    for(i = 0; i < 30; i++ )
    {
        if(integer[ i ] != op2.integer[ i ])//for the == operator,if there is one integer is different,is is !=
            return false;
    }
...
bool HugeInt::operator!=( HugeInt &op2 )
{
    ...
    for(i = 0; i < 30; i++ )
    {
        if(integer[ i ] != op2.integer[ i ])//for the != operator,if there is one integer is different,it is !=,so it's true
            return true;
    }
...
bool HugeInt::operator<( HugeInt &op2 )
{
    ...
    for(i = 0; i < 30; i++ )
    {
        if(integer[ i ] < op2.integer[ i ])//the operator will run to the highest level for comparison.The last will decide the true or false
        return true;
...
/* Write a definition for the > operator
   by calling the <= operator */
bool HugeInt::operator>( HugeInt &op2 )//the operator is similar to the operator <.
{
    int i;

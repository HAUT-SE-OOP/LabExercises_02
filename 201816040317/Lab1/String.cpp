...
String String::operator+( const String &p )/* Write the header for the operator+ member function */
{
   String temp;/* Declare a temporary String variable named temp */
    temp.length=length+p.length;/* Set temp’s length to be the sum of the two argument Strings’ lengthes */
    temp.sPtr= new char[ temp.length + 1 ];/* Allocate memory for temp.length + 1 chars and assign the pointer to temp.sPtr */
    assert( sPtr != 0 ); // terminate if memory not allocated
    strcpy(temp.sPtr,sPtr);/* Copy the left String argument’s contents into temp.sPtr */
    strcat(temp.sPtr,p.sPtr);/* Write a call to strcat to concatenate the string in right

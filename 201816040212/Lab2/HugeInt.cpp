...
bool HugeInt::operator==(HugeInt h1){
    for(int i=30-getLength();i<30;i++){
        if (this->integer[i]!=h1.integer[i]) {
            return false;
            break;
        }
    }
...
/* Write a definition for the < operator */
bool HugeInt::operator<(HugeInt h1){
    if (h1.getLength()>this->getLength()) {
        return true;
    }else{
        for(int i=30-getLength();i<30;i++){
            if (integer[i]==h1.getLength()) {
                continue;
            }else{
                if (integer[i]<h1.integer[i]) {
                    return true;
                    break;
                }else return false;
                }

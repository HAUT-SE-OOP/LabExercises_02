...
// the header for the operator+ member function
const String &String::operator+(const String &op)
{
    String temp;
   // a temporary String variable named temp
    temp.length=length+op.length;
   // Set temp length to be the sum of the two argument Strings?lengthes
   ...
   // Copy the left String argument contents into temp.sPtr
   strcat(temp.sPtr,op.sPtr);
   // a call to strcat to concatenate the string in right onto the end of the string in temp
return temp;

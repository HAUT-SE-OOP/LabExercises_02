...
bool HugeInt::operator<(  const HugeInt &num ) const
{
    if(getLength()<num.getLength())
        return true;
    else if(getLength()==num.getLength())
    {
        for ( int i = 0; i <= 29; i++ )
        {
            if ( integer[ i ] < num.integer[ i ] )
...
HugeInt HugeInt::operator -( const HugeInt &op2 ) const
{
   ...
   for ( int i = 29; i >= 0; i-- )
   {
      ...
      if ( temp.integer[ i ] < 0 )
      {
          if(flag==0)
          {
              temp.integer[ i ] = 10-op2.integer[i]-carry+integer[i];  // reduce to 0-9
          }
          else{temp.integer[ i ] = 10-integer[i]-carry+op2.integer[i];}
   ...
   if(flag==1)
   {
       ...
       temp.integer[30-i]= -temp.integer[30-i];
       return temp;
   }
   else
   {

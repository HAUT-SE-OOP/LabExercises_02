  ...
  const String String::operator+( const String &right )
   {
    ...
    assert(sPtr!=0); //
     //
   strcpy(temp.sPtr,sPtr);// assign new array to sPtr
     strcat(temp.sPtr,right.sPtr);// assign new length to length
      return temp; // enables cascaded calls // end function operator+=

...
/* Implement the RationalNumber constructor. Validate d first to ensure that
   ...
   function at the end */
RationalNumber::RationalNumber(int n, int d) : numerator(n),
                                               denominator(d > 0 ? d : 1)
{
   reduction();
}
...
{
   if (op2.numerator == 0)
      throw string("Exception: cause by:\n devided by zero");
   RationalNumber temp;
...
bool RationalNumber::operator>(const RationalNumber &op2) const
{
   ...
   double b = op2.numerator / op2.denominator;
   return a > b;
}
...
ostream &operator<<(ostream &out, const RationalNumber &rat)
{
